...
{ 
    ...
    for (int clock = 1; num_customers > 0; clock++)
    {
        ...
        while(clock == shopping_q.first_prio())
        {
            ...
            cur_cust->print_done_shopping(os,clock);
            checkout_q.enqueue(cur_cust,1);
        } 
        for(int i = 0;i < num_checkers;i++)
        {
        ...
        for(int i = 0;checkout_q.empty() == false && i < num_checkers;i++)
        { 
            if(checkers[i].m_cust == NULL && checkers[i].m_time_available <= clock)
            {
                Cust *cur_cust = checkout_q.dequeue();                         
